...
<div class="status" id="st">
  ...
  <div class="row"><span class="label">Bumpers F/R</span><span class="val" id="s_bump">--</span></div>
  <div class="row"><span class="label">Cliff F/R</span><span class="val" id="s_cliff">--</span></div>
...
function poll(){
  fetch('/api/status').then(r=>r.json()).then(d=>{
    ...
    let bEl=document.getElementById('s_bump');
    bEl.textContent=(d.bump_front?'HIT':'ok')+' / '+(d.bump_rear?'HIT':'ok');
    bEl.className='val '+((d.bump_front||d.bump_rear)?'off':'on');
    let cEl=document.getElementById('s_cliff');
    cEl.textContent=(d.cliff_front?'DROP':'ok')+' / '+(d.cliff_rear?'DROP':'ok');
    cEl.className='val '+((d.cliff_front||d.cliff_rear)?'off':'on');
    ...
    } else if(d.cliff_front||d.cliff_rear) {
      w.textContent='🚨 CLIFF '+(d.cliff_front?'AHEAD':'BEHIND')+' — back away, then Clear E-Stop';
      w.className='warn danger';
    } else if(d.bump_front||d.bump_rear) {
      w.textContent='⚠ Bumper '+(d.bump_front?'front':'rear')+' pressed';
      w.className='warn';
    } else if(d.obstacle) {
